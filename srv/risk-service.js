...
module.exports = cds.service.impl(async function () {
  ...
  this.after("READ", Risks, (data) => {
   ...
   this.on("READ", Risks, async(req,next)=> {
      /**
       * Check whether the request wants an "expand" of business partner
       * As this is not possible, the risk entity and business partner entity are in different systems (SAP and S/4 HANA Cloud)
      ...
      if(
         !req.query.SELECT.columns.find(
            (column) => column.ref.find((ref) => ref == "bp_BusinessPartner")
         )
      ){
         req.query.SELECT.columns.push({ref:["bp_BusinessPartner"]});
      }
      /**
       * Instead of carrying out the expand, issue a separate request for each biz partner
